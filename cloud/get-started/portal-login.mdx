---
description: >-
  A step-by-step guide to login to your Spice.ai account, connect with your
  Github organizations, and create your first app.
---

# Step 1 - Portal Login

### From the Spice.ai website

1. Go to [spice.ai](https://www.spice.ai) and click on **Login** or **Getting Started** in the top right corner.
2. You can also navigate directly by URL to [spice.ai/login](https://spice.ai/login).

<Frame caption="Login via the Spice.ai website">
  <img src="/images/CleanShot 2023-01-24 at 14.07.06@2x.png" alt="Login via the Spice.ai website" />
</Frame>

3\. Click **Continue with GitHub** to grant authorization to login with your GitHub account.

<Frame caption="Login with GitHub">
  <img src="/images/CleanShot 2023-01-24 at 14.00.46@2x.png" alt="Login with GitHub" />
</Frame>

4\. \*Spice now features [Organization Support](/cloud/portal/organizations). If you have a pre-existing account, you may be prompted to reauthorize with your GitHub. 

5\. Select and click **Grant** next to all the organizations you would like to use with Spice.

6\. Click **Authorize spicehq** to grant Spice AI read-only access to your **Organization and Teams** data.

![](</images/image (6) (1).png>)

**Congrats, you're ready to create your first** [**app**](/cloud/portal/apps)**!**

<Info>
Raise issues or feedback with our team on [Slack](https://spiceai.org/slack).
</Info>
