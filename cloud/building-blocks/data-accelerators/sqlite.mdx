...
Configuration `params` are provided in the `acceleration` section of a dataset. Other common `acceleration` fields can be configured for sqlite, see [datasets](../../reference/spicepod/datasets).
...
- SQLite may not be suitable for high row count use cases with complex join queries. Use [DuckDB](/cloud/building-blocks/data-accelerators/duckdb) instead.
