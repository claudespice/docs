...
While [DuckDB](/cloud/building-blocks/data-accelerators/duckdb) excels for datasets up to approximately 1TB, Cayenne with Vortex is designed to scale beyond these limits.
...
| `cayenne_unsupported_type_action` | Determines what action to take when a data type that is not supported is encountered. See [`unsupported_type_action` for more information](../../reference/spicepod/datasets#unsupported_type_action). |
