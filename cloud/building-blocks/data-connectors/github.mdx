...
- GitHub has a limitation in the Search API where it only returns a maximum of 1000 results for a query. Use [append mode acceleration](/cloud/features/data-acceleration/data-refresh) to retrieve more results over time. See the [append example](#append-example) for pull requests.
</Warning>
