...
| `graphql_query`      | The username to use for basic auth. See [examples](/cloud/building-blocks/data-connectors/graphql#examples) for a sample GraphQL query                                                                                        |
| `json_pointer`       | The [JSON pointer](https://datatracker.ietf.org/doc/html/rfc6901) into the response body. When `graphql_query` is [paginated](/cloud/building-blocks/data-connectors/graphql#pagination), the `json_pointer` can be inferred. |
...
Example for the stargazers query from [pagination section](/cloud/building-blocks/data-connectors/graphql#pagination):
