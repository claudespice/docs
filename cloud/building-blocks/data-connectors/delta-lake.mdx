...
The `from` field for the Delta Lake connector takes the form of `delta_lake:path` where `path` is any supported path, either local or to a cloud storage location. See the [examples](/cloud/building-blocks/data-connectors/delta-lake#examples) section below.
