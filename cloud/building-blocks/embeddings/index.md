...
| -------------------------- | --------------------------------------- | ----------------- | ------------ | ------------------------------- |
| [`file`](local.md)         | Local filesystem                        | Release Candidate | ONNX         | GGUF, GGML, SafeTensor          |
| [`huggingface`](huggingface.md) | Models hosted on HuggingFace       | Release Candidate | ONNX         | GGUF, GGML, SafeTensor          |
| [`openai`](openai.md)      | OpenAI (or compatible) LLM endpoint     | Release Candidate | -            | OpenAI-compatible HTTP endpoint |
...
| [`bedrock`](bedrock.md)    | Models deployed on Amazon Bedrock       | Alpha             | -            | OpenAI-compatible HTTP endpoint |
| [`model2vec`](model2vec.md) | Model2Vec static word embeddings       | Alpha             | -            | Model2Vec format                |
...
- An OpenAI-compatible [endpoint](../../api/HTTP/post-embeddings.md)
...
Spice supports chunking large text columns before embedding, which is useful for [Document Tables](../data-connectors/index.md#document-support). Chunking helps return only the most relevant text during search. Configure chunking in the embedding config:
...
The `row_id` field specifies which column(s) uniquely identify each row, similar to a primary key. This is important for chunked embeddings, so that operations (e.g., [`v1/search`](../../api/HTTP/post-search.md)) can map multiple chunked vectors to a single row. Set `row_id` in `columns[*].embeddings[*].row_id`.
