...
* **Partitioned** [**S3 Vector Indexes**](/cloud/building-blocks/data-connectors/s3) – Improved scalability and query performance for large datasets.
* **AI SQL Function (Preview)** – Query LLMs directly in [SQL](/cloud/portal/playground/sql-query-editor) with `ai()` for summarization, classification, or translation.
...
* Enhanced [vector search](/cloud/use-cases/enterprise-search#vector-similarity-search-across-disparate-and-legacy-data-systems) stability and default limits (now up to 1,000 results).
* Improved AI [SQL query](/cloud/portal/playground/sql-query-editor) consistency and async handling.
...
* [**Hybrid Search (RRF)**](/cloud/use-cases/enterprise-search)**:** Combine [`vector_search`](/cloud/features/search-and-retrieval#vector-search) and `text_search` using the new Reciprocal Rank Fusion (UDTF) for more relevant results; supports per-query rank weights, recency boosting, and configurable decay (`k` = 60.0).
...
*   **AWS Redshift Support:** Connect Redshift clusters using the [PostgreSQL ](/cloud/building-blocks/data-connectors/postgres)data connector (`from: postgres:<schema.table>`).
...
*   **New** [**Data Connectors**](/cloud/building-blocks/data-connectors)**:** Kafka Connector for real-time stream ingestion (`from: kafka:<topic>`).
...
* Fixed [Amazon S3 Vectors](/cloud/building-blocks/data-connectors/s3) API compatibility issue when projecting embedding columns.
...
* **Databricks Integration**
  OAuth for [Databricks](/cloud/integrations/databricks) is fully automatic; your tokens are handled behind the scenes for all catalog, SQL, and chat actions.
...
* **Observability**
  Supports up to 1,000 tasks in [Observability](/cloud/features/observability).
* **Data & Secrets**
  [All secret](/cloud/portal/apps/secrets) names must be uppercase—system now enforces this for new and updated secrets.
...
* **Custom Query Handling**
  [Query arguments](/cloud/reference/sql-reference) included in all GET requests are now passed transparently to downstream systems for more granular filtering and control.
...
* **Monitoring Enhancements**
  [New app and model performance views](/cloud/portal/monitoring-and-request-logs) allow users to see live status and recent activity for the apps and models they own.
...
* **AI Chat & Playground**
  New session handling: users can revisit prior conversations and access a catalog explorer from [Playground](/cloud/portal/playground/ai-chat). Improvements to chat UI display make longer chat sessions and code completions easier to review. Markdown and progress rendering are now supported in chat.
* **Spicepod Editor**
...
* **Copilot & Spicepod Integration**
  [The Copilot onboarding flow](/cloud/integrations/github-copilot) is now unified and leverages shared spicepod instances for organizational consistency. Deployment status and logs are directly viewable in the Copilot UI.
* **Editor Enhancements**
...
The June release included a [C# SDK](/cloud/sdks/dotnet-sdk) for Spice AI and Spice OSS to allow for development using .Net Standard 2.0 and .Net 6.0 or greater.
...
The April release included updates to the Spice SDK's to support querying a local [Spice Runtime](https://github.com/spiceai/spiceai) without requiring a Spice.ai API key.  Details and examples can be found in the [Rust](/cloud/sdks/rust-sdk), [Go](/cloud/sdks/go), [Python](/cloud/sdks/python-sdk), and [Node.js](/cloud/sdks/node.js-sdk) SDK documentation.
...
<Frame caption="Holesky Testnet APIs and datasets available in preview">
  <img src="/images/image (37).png" alt="Holesky Testnet APIs and datasets available in preview" />
</Frame>

**New in this release**

1. \[Platform] Added Ethereum Holesky testnet data and API's.
2. \[Platform] Achieved [SOC 2 Type II compliance](/cloud/security/security).
...
The November release focused on [External Data Sources](/cloud/portal/external-data-sources) and overall performance improvements following the [General Availability (GA)](https://blog.spice.ai/spice-ai-is-generally-available-d76c4289960c) release in October.
...
<Frame caption="New Data Source dialog for connecting external data sources.">
  <img src="/images/image (1) (1) (1).png" alt="New Data Source dialog for connecting external data sources." />
</Frame>
...
<Frame caption="The new navigation command bar.">
  <img src="/images/image (1) (1) (1) (1).png" alt="The new navigation command bar." />
</Frame>

**New In this Release**

1. \[Portal] Added PostgreSQL and MySQL external data sources. See the [docs](/cloud/portal/external-data-sources) for details.
2. \[Portal] Added Dark Mode! Configure via [Profile Settings](/cloud/portal/profile#dark-mode).
...
General Availability launches a brand-new collaborative developer experience, v1 APIs and SDKs, over [100 open-source community](https://github.com/spiceai/datasets) and EigenLayer datasets, and a removal of the waitlist. Any developer can now sign-up and use Spice.ai with one of the new [pricing plans](/cloud/pricing/plans), including the $99 developer plan and the complementary Community Edition.

**New In this Release**

1. \[HTTP API] Add v1 API:
   1. [SQL Query API ](/cloud/api/sql-query/http-api)
   2. Prices API
   3. Gas Fees API
   4. Predictions API
...
4. \[Portal] Datasets and Views generally available, [learn more](/cloud/reference/release-notes#datasets-and-views).
...
The August release expands the functionality of Spice Functions (beta) and [Datasets and Views](/cloud/portal/datasets-and-views).
...
See Broken link for full documentation on creating and deploying Spice Functions.
...
[Spice Firecache](/cloud/features/federated-sql-query) is a real-time SQL cache that enables developers to cache Spice datasets for high concurrency, blazing fast SQL query up to 10x the performance of general SQL query.
...
See [Federated Sql Query](/cloud/features/federated-sql-query) for full documentation on deploying datasets to the Spice Firecache.
...
See [Datasets And Views](/cloud/portal/datasets-and-views) for full documentation on creating private Datasets and Views.
...
See [Connect Github](/cloud/portal/apps/connect-github) for full documentation on connecting your Github Repo to a Spice App.
...
[https://youtu.be/2qk8ZMZOJek](https://youtu.be/2qk8ZMZOJek)
...
<Frame caption="Example Queries referenced in docs are now 1.5x-10x faster!">
  <img src="/images/image (18).png" alt="Example Queries referenced in docs are now 1.5x-10x faster!" />
</Frame>
...
[https://youtu.be/ZJNt3paE7hA](https://youtu.be/ZJNt3paE7hA)
...
<Frame caption="Latest view of all Aave V2 loans query, sample output">
  <img src="/images/Screen Shot 2023-05-09 at 9.49.27 AM.png" alt="Latest view of all Aave V2 loans query, sample output" />
</Frame>
...
<Frame caption="Get ETH withdrawn after the Ethereum Shanghai upgrade, sample output">
  <img src="/images/Screen Shot 2023-05-10 at 10.20.00 PM.png" alt="Get ETH withdrawn after the Ethereum Shanghai upgrade, sample output" />
</Frame>
...
<Frame caption="Recently updated validators, with their balance and status by querying from <code>goerli.beacon.validators</code>, sample output">
  <img src="/images/Screen Shot 2023-05-09 at 8.33.15 PM.png" alt="Recently updated validators, with their balance and status by querying from goerli.beacon.validators, sample output" />
</Frame>
...
1.  ENS Domain Names that map to a validator<br />
    ...
    ![](</images/Screen Shot 2023-04-24 at 10.59.20 PM (1).png>)
...
* Added export to CSV results format in addition to existing formats JSON and [Apache Arrow](/cloud/api/sql-query/apache-arrow-flight-api), for easy use with apps, ML, or libraries like NumPy and Pandas
...
* Broken link for querying Bitcoin data.
...
This update includes a new [Javascript/Typescript SDK](/cloud/sdks/node.js-sdk) for Node.js, an improved [Python SDK](/cloud/sdks/python-sdk), and performance improvements for Ethereum tokens and NFT datasets.
...
* New [Node.js SDK](/cloud/sdks/node.js-sdk)
* Improved [Python SDK](/cloud/sdks/python-sdk)
* ENS support in beta
  * `ens.domains`
...
Performance is now even better, especially for larger queries, and for results over the HTTP API. We still recommend the [Apache Arrow Flight API](/cloud/api/sql-query/apache-arrow-flight-api) that's easily accessible via the [Python SDK](/cloud/sdks/python-sdk) for production use though.
