...
The [Apache Arrow API](api/sql-query-api/apache-arrow-flight-api) uses Apache Arrow Flight to deliver results over a high-performance connection. There is no limit to the number of results you can pull through the Arrow API while the HTTP API is [limited to results of 500 rows](/cloud/limitations).

SDKs like the [Python SDK](/cloud/sdks/python-sdk) always use the Arrow API, so they are a convenient way to access Spice data.
