...
{% hint style="info" %}
**IAM Permissions:** Regardless of the credential source, the IAM role or user must have appropriate S3/Glue permissions (e.g., `s3:ListBucket`, `glue:GetTable`) to access the tables. If the Spicepod connects to multiple different AWS services, the permissions should cover all of them.
{% endhint %}
...
| Permission | Purpose |
|------------|---------|
| `s3:ListBucket` | Required. Scanning all objects from the bucket |
| `s3:GetObject` | Required. Fetching objects |
