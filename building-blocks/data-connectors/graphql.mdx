...
| `graphql_query`      | The username to use for basic auth. See [examples](graphql#examples) for a sample GraphQL query                                                                                        |
| `json_pointer`       | The [JSON pointer](https://datatracker.ietf.org/doc/html/rfc6901) into the response body. When `graphql_query` is [paginated](graphql#pagination), the `json_pointer` can be inferred. |
...
Example for the stargazers query from [pagination section](graphql#pagination):
